...
public UserController(IUsersBL _repo)
{
this.repo = _repo;
}
...
{ 
 ...
 try
 {
 data =  repo.GetUsersById(Id).Result;
 }
catch (Exception ex) 
{ 
 throw;
...
try 
 {
result = await repo.UpdateUsers(data); 
 }
catch (Exception ex) 
{ 
 throw;
...
{ 
 ...
 public async Task<JsonResult> DropDownUsers() 
 {
   dynamic result;
 try
 {
  result = await repo.DropDownUsers();
 }
catch (Exception ex) 
 {
  throw ex;
