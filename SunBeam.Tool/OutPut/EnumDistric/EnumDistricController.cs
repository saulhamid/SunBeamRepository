...
{
public class EnumDistriController:Controller
{
private readonly IEnumDistricBL repo;
public EnumDistriController(IEnumDistricBL _repo)
{
this.repo = _repo;
}
...
[HttpPost] 
  public async Task<ActionResult> Create(EnumDistric data)
{ 
 string result = string.Empty; 
try 
 {
result = await repo.InsertEnumDistric(data); 
 }
catch (Exception ex) 
{ 
 throw;
...
{ 
 ...
 try
 {
 data =  repo.GetEnumDistricById(Id).Result;
 }
catch (Exception ex) 
{ 
 throw;
...
{ 
 ...
 public async Task<JsonResult> DropDownEnumDistric() 
 {
   dynamic result;
 try
 {
  result = await repo.DropDownEnumDistric();
 }
catch (Exception ex) 
 {
  throw ex;
